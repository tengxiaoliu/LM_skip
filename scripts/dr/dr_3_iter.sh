...
do
  ...
  echo "========================================================================"
  echo "=== Iteration for dr iter${ITER_NUM} -> iter$((ITER_NUM + 1)) ==="
  echo ">>> 1. Inference on train set"

  export VLLM_WORKER_MULTIPROC_METHOD=spawn

  MODEL_DIR="outputs/phi/dr/model/phi_num_iter${ITER_NUM}"
  MODEL_NAME="microsoft/Phi-3-mini-4k-instruct"
  TOKENIZER_NAME="microsoft/Phi-3-mini-4k-instruct"
  ALIAS="phi"
  DATA_SAVE_PATH="outputs/phi/dr/data"

  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
  python src/vllm_offline_inference.py \
      --data_path  "data/DR/train_cold.jsonl" \
      --model_name ${MODEL_DIR} \
      --save_path ${DATA_SAVE_PATH} \
      --tokenizer_name ${TOKENIZER_NAME} \
      --dataset_name dr_train \
  ...
  python src/prepare_sft_data_general.py \
      --raw_file "data/DR/train_cold.jsonl" \
      --pred_files "${DATA_SAVE_PATH}/phi_iter${ITER_NUM}_dr_train_${ALIAS}_skip-1_preds.jsonl" \
                   "${DATA_SAVE_PATH}/phi_iter${ITER_NUM}_dr_train_${ALIAS}_skip-2_preds.jsonl" \
  ...
  accelerate launch --config_file=accelerate_configs/deepspeed_zero3.yaml --num_processes 8 --main_process_port 12306 src/sft.py \
      --do_train \
      --train_dataset "${DATA_SAVE_PATH}/phi_iter${ITER_NUM}_dr_train_${ALIAS}_skip-2_new_sft_data.jsonl" \
      --max_seq_length 8000 \
  ...
  MODEL_PATH="outputs/phi/dr/model/phi_num_iter$((ITER_NUM + 1))"
  OUTPUT_PATH="outputs/phi/dr/test"
  ...
  echo "=== DONE Iteration for dr iter${ITER_NUM} -> iter$((ITER_NUM + 1)) ==="
  echo "=========================================================================DONE"
